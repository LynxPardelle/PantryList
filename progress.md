...
| Social auth full audit | Backend lint/unit/e2e/build, frontend unit/build/e2e, infra build/synth, prod audits, and secret scan | Confirm social auth changes are locally safe after deploy | Backend `48` tests passed; frontend `18` tests and `1` Playwright e2e passed; prod audits and secret scan returned `0` findings | ✓ |
| Backend dev dependency audit fix | `npm install --save-dev @swc/cli@^0.8.1` then `npm audit --json` in `backend/` | Clear backend dev audit findings without changing runtime dependencies | Full backend audit returned `total = 0` | ✓ |
| Cognito Managed Login branding deploy | `npx cdk deploy --require-approval never ...` in `infra/cognito/` | App client has a default Managed Login style so hosted pages render | CloudFormation created `AWS::Cognito::ManagedLoginBranding`; stack returned `UPDATE_COMPLETE` | ✓ |
| Browser smoke after Managed Login branding | Playwright opened `http://localhost:48673/login` and clicked Cognito, Facebook, and Google buttons | Validate real provider handoff without entering credentials | Cognito rendered hosted sign-in; Facebook reached login; Google blocked with `redirect_uri_mismatch` until Google OAuth redirect URI is configured | ⚠ |
| Google OAuth redirect re-smoke | Playwright opened `http://localhost:48673/login` and clicked `Entrar con Google` | Confirm the Google OAuth Console redirect URI change fixed the previous blocker | Google reached `accounts.google.com/v3/signin/identifier` with Cognito `/oauth2/idpresponse` as `redirect_uri`; no `redirect_uri_mismatch` observed | ✓ |
| Angular 21 version verification | `npm view @angular/core version`, `npm view @angular/cli version`, and `npx ng version` in `frontend/` | Confirm current stable target and installed versions | npm returned core `21.2.10` and CLI `21.2.8`; installed Angular `21.2.10`, CLI `21.2.8` | ✓ |
| Angular 19 to 21 migration | `ng update` from 19 to 20, then 20 to 21 with Angular/Material/SSR/NgRx updates | Move frontend to the current supported Angular major while running official migrations | Package versions, `angular.json`, `src/main.ts`, and HTTP client providers updated successfully | ✓ |
| Angular 21 frontend verification | `npm run test:ci`, `npm run build`, `npm run test:e2e`, `npm audit --omit=dev --json` | Verify Angular 21 app behavior and production dependency risk | `18` Karma specs passed, build/prerender passed, `1` Playwright e2e passed, production audit returned `total = 0` | ✓ |
| Angular 21 Docker verification | `docker compose --env-file .env.docker.local --profile app up -d --build frontend`, container logs, and Playwright smoke | Ensure local Docker frontend runs with Angular 21 dependencies | Docker image built, runtime container started, logs no longer show Angular SSR fetch warning, browser smoke saw Cognito/Google/Facebook buttons | ✓ |
| Cognito duplicate-email callback fix | Backend profile sync now maps Cognito `sub` values through `authSubjectIds` instead of assuming one `sub` equals one PantryList user | Allow the same verified email to enter through Cognito/social providers without hitting Mongo unique `email` conflicts | Targeted Cognito/guard/controller tests passed (`15` tests), full backend suite passed (`50` tests), backend build passed, Docker backend rebuilt, `/api/healthz` returned `200` | ✓ |
| Profile preferences design context | `.impeccable.md` | Capture product design direction before profile/settings UX work | Defined PantryList as `Hogar operativo`: practical, calm, configurable; explicitly avoids corporate SaaS, childlike UI, and medical/alarm aesthetics | ✓ |
| Profile preferences and expired alerts spec | `docs/superpowers/specs/2026-04-28-profile-preferences-expired-alerts-design.md` | Write approved design before implementation | Spec covers `/profile`, user preferences, distinct `expired` status, non-modal expired entry alert, architecture, accessibility, and tests | ✓ |
| Backend profile preferences implementation | User preferences value object, DAO port/adapter, profile controller, overview builder, expiration status, and tests | Persist user-scoped settings and make expired/soon behavior configurable | Targeted Jest suite passed (`17` tests), full backend suite passed (`22` suites, `64` tests), backend build passed | ✓ |
| Frontend profile preferences implementation | Lazy `/profile` page, `ProfileService`, pantry expired entry alert, preference-aware pantry overview mapping, and specs | Let users configure pantry behavior and see expired inventory immediately on entry | Frontend `npm run test:ci` passed (`24` tests), `npm run build` passed with initial total `493.65 kB`, Playwright E2E passed (`3` tests) | ✓ |
| Docker app stack after profile preferences | `docker compose --env-file .env.docker.local --profile app up -d --build` and HTTP smoke | Run MongoDB, backend, and frontend together after the feature | MongoDB healthy on `37917`; backend `http://localhost:39173/api/healthz` returned `200`; frontend `http://localhost:48673/login` returned `200` | ✓ |
| Profile preferences production dependency audits | `npm audit --omit=dev --json` in backend and frontend | Confirm no production dependency vulnerabilities introduced | Both production audits returned `total = 0` | ✓ |
| Profile preferences secret scan | Changed-file pattern scan plus `python C:\Users\lince\.codex\skills\security-compliance\scripts\secret_scan.py . --json` | Confirm no likely secrets were added to tracked/untracked feature files | Changed-file scan only found config-name false positives; `security-compliance` returned `count = 0` and `findings = []` | ✓ |
| Profile preferences whitespace check | `git diff --check` | Catch whitespace errors before handoff | Passed; Git only printed CRLF normalization warnings | ✓ |
| Facebook Cognito state RED test | `npx jest src/infrastructure/http/controllers/auth.controller.spec.ts src/infrastructure/http/auth/auth-cookie.service.spec.ts --runInBand` before implementation | Prove stale callback state currently throws JSON-facing `401` | Failed on `UnauthorizedException: Invalid Cognito auth state`, confirming the missing recovery path | ✓ |
| Facebook Cognito callback fix tests | Backend targeted Jest, frontend `npm run test:ci`, backend build, frontend build, and Playwright E2E | Verify longer TTL, invalid-state recovery redirect, and login error messaging | Backend targeted tests passed (`8` tests); frontend tests passed (`24` tests); backend/frontend builds passed; E2E passed (`4` tests) | ✓ |
| Facebook Cognito callback runtime smoke | Docker app rebuild plus `curl.exe -i` against login and stale callback URLs | Verify Docker picked up local TTL and callback recovery behavior | Facebook login set `pantrylist_cognito_state` with `Max-Age=900`; stale callback returned `302` to `/login?authError=cognito_state` and cleared transaction cookies | ✓ |
| Facebook Cognito callback security checks | `npm audit --omit=dev --json` in backend/frontend and `security-compliance` secret scan | Ensure the auth fix did not add production dependency or secret risk | Backend and frontend production audits returned `total = 0`; secret scan returned `count = 0` and `findings = []` | ✓ |
| Expiration split frontend tests | `npm run test:ci` in `frontend/` | Verify expired vs upcoming quantity helpers, singular quantity formatting, and existing component behavior | `TOTAL: 26 SUCCESS` | ✓ |
| Expiration split frontend build | `npm run build` in `frontend/` | Confirm Angular compiles after template/date display changes | Build/prerender completed; initial total `497.40 kB` | ✓ |
| Expiration split Docker frontend rebuild | `docker compose --env-file .env.docker.local --profile app restart frontend` after source changes | Force the Docker dev server to pick up the latest template changes | Frontend rebuilt at `2026-04-29T06:35:56.182Z`; dev server listening on container port `4200` and host port `48673` | ✓ |
| Expiration split E2E | `$env:E2E_BASE_URL='http://localhost:48673'; npm run test:e2e` in `frontend/` | Verify the browser shows separate `Ya caducó` and `Por caducar` counts plus exact expired lot dates | `4 passed` | ✓ |
| Expiration split security checks | `npm audit --omit=dev --json` in `frontend/` plus `security-compliance` secret scan | Confirm no production dependency or secret risk was introduced by the UI change | Frontend production audit returned `total = 0`; secret scan returned `count = 0` and `findings = []` | ✓ |
| Replenishment feature intake | User report plus code inspection of `depletion-forecast.service.ts`, `pantry-overview.builder.ts`, `ConsumeInventoryLotUseCase`, profile preferences, pantry models, and UI templates | Treat durability aging, zero-stock shopping plans, criticity labels, purchase-date visibility, per-type overrides, copy, and tutorial guidance as one planned feature | Root causes identified and Phase 20 opened in `task_plan.md` | ✓ |
| Replenishment design spec | `docs/superpowers/specs/2026-04-29-replenishment-intelligence-guided-ux-design.md` | Capture the approved hybrid approach plus safe archive/delete flows before implementation planning | Spec written and self-reviewed; user approved the hybrid approach plus archive/delete direction | ✓ |
| Replenishment implementation plan | `plan/feature-replenishment-intelligence-1.md` | Break the approved design into backend, frontend, archive, guidance, and verification tasks | Detailed 45-task implementation plan created; runtime implementation remains pending after this commit boundary | ✓ |
| Replenishment backend implementation | Product type planning settings, archive/restore/delete use cases, archived read model, purchase-date lot summaries, product-type-first overview, lot purchase-date depletion, zero-stock shopping plans, and controller/DAO wiring | Fix product disappearance after zero stock and make durability/read models match household replenishment decisions | Full backend suite passed: `30` suites, `95` tests; backend build passed | ✓ |
| Replenishment frontend implementation | Pantry models/service, profile `showGuidanceTips`, pantry guidance, purchase-date display, per-type planning UI, archive panel, action-oriented copy, and lazy-loaded `PantryModule` | Expose the new backend behavior with clearer household UX while keeping initial bundle under budget | Frontend tests passed: `31` Karma specs; build passed with initial bundle `457.00 kB` and lazy `pantry-module` `56.93 kB` | ✓ |
| Replenishment Docker and E2E verification | `docker compose --env-file .env.docker.local --profile app up -d --build`, backend/frontend HTTP smoke, and `$env:E2E_BASE_URL='http://localhost:48673'; npm run test:e2e` | Verify the containerized app serves the new Angular chunk and backend API behavior | Docker app profile rebuilt; backend health returned `200`; frontend `/login` returned `200`; Playwright E2E passed `5` tests | ✓ |
| Replenishment security checks | `npm audit --omit=dev --json` in backend/frontend plus `python C:\Users\lince\.codex\skills\security-compliance\scripts\secret_scan.py . --json` | Confirm no production dependency or secret risk was introduced by archive/planning work | Backend production audit `total = 0`; frontend production audit `total = 0`; secret scan `count = 0` and `findings = []` | ✓ |
| Replenishment whitespace and e2e checks | `git diff --check` and `npm run test:e2e` in backend | Catch whitespace and API e2e regressions before commit | `git diff --check` passed with CRLF normalization warnings only; backend e2e passed `2` tests | ✓ |
| Production AWS deployment intake | User request, local deployment docs, `docker-compose.prod.yml`, AWS CLI preflight | Identify exact production target before provisioning resources | Target domain is `pantrylist.lynxpardelle.com`; future environments are `test.pantrylist.lynxpardelle.com` and `dev.pantrylist.lynxpardelle.com`; AWS account is `765932874577` in `us-east-1`; Dokploy runs on EC2 `i-061f471ff5edea8a9`; production persistence must be DynamoDB, not MongoDB | ✓ |
| Production DynamoDB implementation | `@aws-sdk/client-dynamodb`, `@aws-sdk/lib-dynamodb`, DynamoDB DAO/repository adapters, `AppModule` provider selection, backend tests/build | Make production persistence use DynamoDB without changing controllers/use cases | Backend unit tests passed `30` suites / `95` tests; backend e2e passed `2` tests; backend build passed | ✓ |
| Production frontend and security verification | Frontend tests/build, production dependency audits, and `security-compliance` secret scan | Confirm the deployable code path is clean before provisioning AWS resources | Frontend unit tests passed `31` specs; frontend build passed; backend/frontend production audits returned `total = 0`; secret scan returned `count = 0` and `findings = []` | ✓ |
| Production AWS CDK deploy | `npx cdk deploy pantrylist-prod-cognito pantrylist-prod-app ... --context includeProductionInfra=true` | Create production Cognito, DynamoDB, ACM, CloudFront, Route53, and EC2 IAM permissions reproducibly | CloudFormation created both stacks; Cognito User Pool `us-east-1_H5nTCoqqg`; CloudFront distribution `E244X3QM2RVQYC`; DynamoDB tables `pantrylist-prod-users`, `pantrylist-prod-products`, `pantrylist-prod-product-types`, and `pantrylist-prod-inventory-lots` | ✓ |
| Production Dokploy EC2 runtime deploy | AWS SSM command to the existing Dokploy EC2 host with `docker-compose -p pantrylist-prod -f docker-compose.prod.yml -f docker-compose.dokploy.yml --env-file .env.production.local up -d --build` | Run `main` with production Cognito and DynamoDB on the existing Dokploy server | Backend container `pantrylist-prod-backend-1` and frontend container `pantrylist-prod-frontend-1` reported healthy; Traefik dynamic route was added for `pantrylist.lynxpardelle.com` | ✓ |
| Production external smoke | `curl.exe -k` checks against `https://pantrylist.lynxpardelle.com` | Verify CloudFront, Route53, Traefik, frontend, backend API, and Cognito provider configuration from outside EC2 | `/healthz`, `/api/healthz`, `/login/`, and `/api/auth/cognito/providers` returned `200`; providers endpoint returned `{"providers":["COGNITO","Google","Facebook"]}`; Hosted UI rendered | ✓ |
| Production AWS report | `docs/deployment/aws-production-report.md` | Record exactly what was created, how it was verified, remaining actions, and cost sources | Report added with AWS resources, verification results, residual risks, and official AWS pricing links | ✓ |
| Dokploy CI/CD compose preparation | `docker-compose.dokploy.prod.yml`, `docker compose -f docker-compose.dokploy.prod.yml config --quiet`, docs runbook | Give Dokploy a single production compose path that can be loaded from GitHub without publishing host ports | Compose config validation passed; runbook added at `docs/deployment/dokploy-cicd-cloudfront-origin-hardening.md`; pushed to `main` | ✓ |
| Dokploy project/service creation | Dokploy API with temporary API key, `project.create`, `compose.create`, `compose.update`, `domain.create`, `compose.deploy` | Make production visible and deployable from Dokploy UI with GitHub `main` auto deploy | Project `PantryList`, compose `pantrylist-production`, GitHub source `LynxPardelle/PantryList`, branch `main`, compose path `docker-compose.dokploy.prod.yml`, and auto deploy `true`; deployment status `done` | ✓ |
| CloudFront origin HTTPS hardening | CDK deploys with `originRecordName`, `originProtocolPolicy`, `originRequestPolicy`, and `enableIpv6` controls | Encrypt CloudFront-to-EC2/Traefik traffic while keeping the public domain unchanged | CloudFront distribution `E244X3QM2RVQYC` is `Deployed`; origin is `origin.pantrylist.lynxpardelle.com`; origin protocol policy is `https-only`; origin request policy id is `b689b0a8-53d0-40ab-baf2-68738e2966ac`; IPv6 is disabled | ✓ |
| Post-hardening production smoke | Repeated `curl.exe --ssl-no-revoke` checks against public and origin URLs | Confirm HTTPS origin and Dokploy-managed service are serving production reliably | Ten repeated public checks returned `200` for `/api/healthz` and `/api/auth/cognito/providers`; `/login/` returned `200`; direct origin `/api/healthz` returned `200`; Cognito Hosted UI launch returned `200` with Cognito cookies | ✓ |
| Dokploy UI cleanup | Dokploy domain API and compose deploy | Show `pantrylist.lynxpardelle.com` inside the `PantryList` project and remove the origin domain from the UI domain list | Dokploy compose `pantrylist-production` now lists only `pantrylist.lynxpardelle.com`, service `frontend`, port `4000`, auto deploy `true` | ✓ |
| Manual stack cleanup | SSM command running `docker-compose -p pantrylist-prod ... down --remove-orphans` plus Traefik file removal | Ensure there are no unlisted PantryList containers outside the Dokploy-managed project | Removed `pantrylist-prod-frontend-1`, `pantrylist-prod-backend-1`, network `pantrylist-prod_pantrylist-network`, and `/etc/dokploy/traefik/dynamic/pantrylist-prod.yml`; inventory showed only `compose-compress-back-end-port-hiewlq-*` PantryList containers | ✓ |
| CloudFront-only origin restriction | Secrets Manager origin verification header, CloudFront custom header, protected Traefik origin route, and direct public-host block route | Prevent direct origin access while preserving CloudFront production traffic | Public `/api/healthz` and `/login/` returned `200` in repeated checks; direct `origin.pantrylist.lynxpardelle.com` without header returned `404`; direct EC2 HTTP with `Host: pantrylist.lynxpardelle.com` returned `502`; CloudFront distribution showed custom header `X-PantryList-Origin-Verify` configured | ✓ |
...
| 2026-04-27 Central Time | Cognito hosted login returned `Login pages unavailable` after social-provider deploy | 1 | Added `AWS::Cognito::ManagedLoginBranding` with `UseCognitoProvidedValues=true`, redeployed CDK, and verified the hosted sign-in page renders |
| 2026-04-27 Central Time | Google social login reached Google but failed with `redirect_uri_mismatch` | 1 | Pending external Google Console change: add `https://pantrylist-dev-765932874577.auth.us-east-1.amazoncognito.com/oauth2/idpresponse` as an authorized redirect URI |
| 2026-04-27 Central Time | Angular 21 `ng update` initially failed because `ngx-bootstrap@19` required Angular `^19` peers | 1 | Ran Angular 21 update with `--force` only to cross the temporary peer mismatch, then immediately upgraded `ngx-bootstrap` to `21.2.0` |
| 2026-04-27 Central Time | First Angular 21 Docker frontend build failed because `npm ci` found package-lock mismatch for `@types/node`/`undici-types` | 1 | Aligned frontend dev dependency `@types/node` with the Node 22 line (`^22.19.17`), regenerated the lockfile, and rebuilt Docker successfully |
| 2026-04-28 Central Time | Cognito hosted callback returned `500 Internal server error` after account verification because Mongo rejected a duplicate unique `email` during profile sync | 1 | Root cause was treating each Cognito `sub` as a separate PantryList user; added `authSubjectIds` identity linking, guarded email linking behind verified Cognito email claims, updated access-token lookup to resolve by auth subject, rebuilt backend Docker, and verified health/tests |
| 2026-04-28 Central Time | First Brainstorm visual companion launch failed because this PowerShell session rejected `New-Item -LiteralPath` for directory creation | 1 | Retrying with `New-Item -Path` before falling back to text-only wireframes if the companion cannot stay running |
| 2026-04-28 Central Time | Docker daemon was initially unavailable during the profile preferences validation pass | 1 | Started Docker Desktop hidden, waited for Docker readiness, rebuilt the `app` profile, and verified backend/frontend HTTP health |
| 2026-04-28 Central Time | First changed-file secret scan command failed because the PowerShell file list was a nested `System.Object[]` | 1 | Re-ran with a flattened string path array and then used the `security-compliance` scanner, which returned zero findings |
| 2026-04-28 Central Time | One frontend `npm run test:ci` run showed transient ChromeHeadless temp-directory/crashpad startup noise | 1 | Re-ran the frontend test workflow and confirmed the suite passed |
| 2026-04-29 Central Time | Facebook callback returned `{"message":"Invalid Cognito auth state","error":"Unauthorized","statusCode":401}` | 1 | Root cause was a stale, expired, or overwritten OAuth transaction cookie. Kept strict state validation, extended the transaction TTL to 900 seconds, redirected stale callbacks to `/login?authError=cognito_state`, cleared transaction cookies, and verified with Docker runtime smoke plus E2E. |
| 2026-04-29 Central Time | First expiration split E2E expected `20 Apr 2026`, but Angular rendered `19 Apr 2026` from `2026-04-20T00:00:00.000Z` | 1 | Root cause was local timezone formatting of a date-only UTC-midnight value. Rendered pantry calendar dates with UTC formatting, restarted the frontend Docker watcher, and reran E2E successfully. |
| 2026-04-29 Central Time | Durable product purchased one month ago still showed full quantity and a depletion date one month in the future | 1 | Root cause is that depletion is anchored to the product-type rule date, not lot purchase date. Needs a feature design decision: per-lot aging against purchase date, type-level rolling consumption, or a hybrid stock ledger. |
| 2026-04-29 Central Time | Product disappears from plan de compras after manually consuming the final active lot | 1 | Root cause is that empty lots are deleted and pantry overview groups are built only from active lots. Needs a zero-stock/restock read model based on active product types with depletion rules. |
| 2026-04-29 Central Time | No Dokploy API key was available locally for creating a first-class Dokploy app | 1 | Deployed through AWS SSM onto the existing Dokploy EC2 host using Docker Compose and a Traefik dynamic route; documented Dokploy API/UI recreation as a remaining production action. |
| 2026-04-29 Central Time | The EC2 host supported `docker-compose` but not the newer `docker compose` command | 1 | Retried deployment with `docker-compose -p pantrylist-prod ... up -d --build`, which completed successfully. |
| 2026-04-29 Central Time | CloudFront initially reached the EC2 origin but returned `404` | 1 | Added `/etc/dokploy/traefik/dynamic/pantrylist-prod.yml` so Traefik routes `pantrylist.lynxpardelle.com` to the PantryList frontend container. |
| 2026-04-29 Central Time | AWS CLI SSM output hit a Windows character encoding error while streaming Docker build output | 1 | Queried the SSM command result afterward with `PYTHONIOENCODING=utf-8` and verified the deployment state from Docker, Traefik, and external HTTP checks. |
| 2026-04-29 Central Time | `compose.saveEnvironment` returned `404 Not found` on the installed Dokploy API version | 1 | Saved the compose environment through `compose.update` using the `env` field, then verified the compose had 27 non-empty environment lines. |
| 2026-04-29 Central Time | First CloudFront HTTPS-origin cutover returned `502 Bad Gateway` | 1 | Rolled back immediately to HTTP-only, verified public health returned `200`, then updated CDK to use `ALL_VIEWER_EXCEPT_HOST_HEADER` in HTTPS-origin mode before redeploying. |
| 2026-04-29 Central Time | Repeated CloudFront IPv6 requests showed intermittent TLS resets | 1 | Made CloudFront IPv6 configurable, redeployed production with `enableIpv6=false`, removed the Route53 AAAA alias, and verified ten repeated public checks returned `200`. |
| 2026-04-29 Central Time | First protected-origin Traefik route script exposed the old verification value in SSM stderr because `/bin/sh` interpreted backticks | 1 | Rotated the Secrets Manager value immediately, redeployed CloudFront, rewrote the route with escaped double quotes, and verified the second SSM command had empty stderr. |
| 2026-04-29 Central Time | Dokploy compose domain update did not persist the documented `middlewares` field | 1 | Added a higher-priority Traefik route to block direct public-host HTTP access while keeping `pantrylist.lynxpardelle.com` visible in the Dokploy UI. |
