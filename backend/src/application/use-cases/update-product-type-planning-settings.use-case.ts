...
import {
  ProductType,
  ProductTypePlanningSettingsPatch,
...
export interface UpdateProductTypePlanningSettingsCommand {
  productTypeId: string;
  userId: string;
  planningSettings: ProductTypePlanningSettingsPatch;
}

@Injectable()
export class UpdateProductTypePlanningSettingsUseCase {
  constructor(
  ...
  async execute(
    command: UpdateProductTypePlanningSettingsCommand,
  ): Promise<ProductType> {
    ...
    productType.updatePlanningSettings(command.planningSettings);
